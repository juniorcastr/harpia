...
class PontoRemotoService
{
    ...
    public function registrar(Colaborador $colaborador, string $tipo, array $contexto = []): EventoAcesso
    {
        if (!in_array($tipo, ['entrada', 'saida'], true)) {
            throw new \InvalidArgumentException('Tipo de registro remoto inválido.');
        }
        ...
        $evento = EventoAcesso::create([
            'eva_matricula' => (string) $colaborador->col_id,
            ...
            'eva_status' => 'pendente',
            'eva_status_mensagem' => 'Registro remoto aguardando aprovação do gestor.',
        ...
        Log::channel('ponto')->info('Registro remoto criado e enviado para aprovação.', [
    ...
    public function obterEstadoAtualDoDia(Colaborador $colaborador, ?Carbon $data = null): array
    {
        ...
        $eventos = $this->eventoRepository->buscarEfetivosDoDia($colaborador->col_id, $data);
    ...
    {
        return $this->eventoRepository->listarDoColaborador($colaboradorId, $limite);
    }
    ...
    private function validarAprovadores(Colaborador $colaborador): void
    {
        if ($this->aprovadorPontoService->resolverAprovadores($colaborador)->isEmpty()) {
            throw new \InvalidArgumentException('Não foi encontrado gestor responsável para aprovar este registro remoto.');
        }
    }

    private function validarJanelaHorario(Carbon $dataHora): void
    {
        $janelaInicio = (string) $this->configuracaoRepository->obterValor('ponto.janela_inicio', '05:00');
        $janelaFim = (string) $this->configuracaoRepository->obterValor('ponto.janela_fim', '23:00');
        $horaAtual = $dataHora->format('H:i');

        if ($horaAtual < $janelaInicio || $horaAtual > $janelaFim) {
            throw new \InvalidArgumentException(sprintf(
                'O registro remoto está fora da janela permitida. Horário válido: %s às %s.',
                $janelaInicio,
                $janelaFim
            ));
        }
    ...
    private function validarSequencia(Colaborador $colaborador, string $tipo, Carbon $dataHora): void
    {
        ...
        if ($tipo === 'entrada' && $estado['tem_entrada_aberta']) {
            throw new \InvalidArgumentException('Já existe uma entrada em aberto para o dia atual.');
        }

        if ($tipo === 'saida' && !$estado['tem_entrada_aberta']) {
            throw new \InvalidArgumentException('Não existe entrada em aberto para registrar a saída.');
        }
    }

    private function validarAntiDuplicidade(Colaborador $colaborador, string $tipo, Carbon $dataHora): void
    {
        $janelaSegundos = (int) $this->configuracaoRepository->obterValor('ponto.anti_duplicidade_seg', 60);
        ...
        if ($duplicadoRecente) {
            throw new \InvalidArgumentException('Já existe um registro remoto recente deste tipo. Aguarde alguns instantes para tentar novamente.');
        }
        ...
        if ($this->antiDuplicidadeService->existeHash($hash)) {
            throw new \InvalidArgumentException('Já existe um registro remoto equivalente para este minuto.');
        }
    }
